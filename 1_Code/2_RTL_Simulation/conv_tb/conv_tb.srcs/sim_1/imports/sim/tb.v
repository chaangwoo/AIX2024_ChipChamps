...
initial begin
	...
	$display ("Total cycle: %0d", read_f_cyc + read_b_cyc + read_w_cyc + compute_cyc);
	$display ("    READ  | f: %0d, b: %0d, w: %0d", read_f_cyc, read_b_cyc, read_w_cyc);
	$display ("  COMPUTE | %0d\n", compute_cyc);
...
wire [31:0] mask;
assign mask = is_1x1 ? (is_last ? 32'd24 : (No >> 3) - 1) : ((No >> 4) - 1);
